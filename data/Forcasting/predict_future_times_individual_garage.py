...
def load_instruction_days(data):
...
def cyclical_time_encoding(data):
    ts = data['date']
    ...
    return data.drop(columns=["date"]).copy()

# long model hyperparameters
def _build_long_model(garage_no, shape):
    return build_model(
...
def _build_short_model(garage_no, shape):
    return build_model(
...
def _train_long(garage: str, long_garage_model):
...
def _train_short(garage: str, short_garage_model):
...
def _make_prediction(data, short_data, long_garage_models, short_garage_models, short_feature_shape, long_feature_shape):
    # Scale the data for long model
    scaler_long = MinMaxScaler()
    scaler_long.fit(data)
    scaled_long = pd.DataFrame(scaler_long.transform(data), columns=data.columns)
...
def main():
    global extra_long_data
    data = load_data()
    short_data = data.drop(columns=["date"]).copy() # Keep a copy of the raw density data (without date)
    long_garage_models = []
    short_garage_models = []
