...
# Automatically configure device based on OS
if platform.system() == "Linux":
    # Use GPU if available on Ubuntu
    physical_devices = tf.config.list_physical_devices('GPU')
    if physical_devices:
        tf.config.experimental.set_memory_growth(physical_devices[0], True)
        print("Using GPU on Ubuntu")
    else:
        print("No GPU found, using CPU on Ubuntu")
else:
    # Force CPU usage on Windows
    tf.config.set_visible_devices([], 'GPU')
    print("Using CPU on Windows")
...
if physical_devices:
    ...
    tf.config.experimental.set_memory_growth(physical_devices[0], True)
    print("Using GPU with mixed precision training")

    # Enable mixed precision
    set_global_policy('mixed_float16')
else:
    print("No GPU found, skipping GPU optimizations")
